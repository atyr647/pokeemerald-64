 ...
 *
 * The GBA renders at 240×160, which sits in the centre of the 320×240 VI
 * framebuffer with a black border around it.  The compositor used to draw
 * into a separate 240×160 buffer that was then copied in, which cost a
 * 38,400-pixel read-modify-write every frame -- a sixth to a third of the
 * whole frame.  It writes straight into the back buffer now: gN64FrameBuf
 * points at the image's top-left pixel and rows are N64_VI_WIDTH apart
 * rather than DISPLAY_WIDTH.  The border is painted once per buffer and
 * never touched again.
 *
 ...
 *
 * Cache: the compositor's writes go through KSEG0 so they land in the data
 * cache, and the whole cache is written back before VI_ORIGIN flips.  The
 * alternative -- writing uncached, which is what the old blit did -- costs
 * a full RDRAM round trip per store on real hardware, and the compositor
 * makes 38,400 of them per frame.
 *
 * Endianness note:
...
/* Back buffer (compositor writes here), cached */
u16 *gN64BackBuffer  = NULL;
u16 *gN64FrontBuffer = NULL;

/* Where the 240x160 picture starts inside the back buffer. The compositor
 * writes here directly; rows are N64_VI_WIDTH apart. */
u16 *gN64FrameBuf = NULL;

static void N64_SetBackBuffer(u16 *back)
{
    gN64BackBuffer = back;
    gN64FrameBuf   = back + N64_FB_Y_OFFSET * N64_VI_WIDTH + N64_FB_X_OFFSET;
}

/* Write the data cache back to RDRAM so the VI sees this frame.
 *
...
{
    /* KSEG0 (cached): the compositor writes far too much per frame to pay
     * for uncached stores, so the cache is written back once per frame in
     * N64_VISwapBuffers() instead.                                        */
    gN64FrontBuffer = (u16 *)__fb0_start;
    N64_SetBackBuffer((u16 *)__fb1_start);
    sDisplayFB      = 0;
...
void N64_VISwapBuffers(void)
{
    ...
    /* The frame only exists in the data cache until this runs. */
    N64_WritebackDCache();

    VI_WR(VI_ORIGIN_REG, (u32)((uintptr_t)newFront & 0x00FFFFFF));

    if (sDisplayFB == 0) {
        sDisplayFB      = 1;
        gN64FrontBuffer = (u16 *)__fb1_start;
        N64_SetBackBuffer((u16 *)__fb0_start);
    } else {
        sDisplayFB      = 0;
        gN64FrontBuffer = (u16 *)__fb0_start;
        N64_SetBackBuffer((u16 *)__fb1_start);
...
/* The letterbox is the same every frame and the compositor never writes
 * outside the picture, so it is painted once per buffer.
 *
 * Not in N64_InitVI(): N64Main()'s boot diagnostics fill the whole
 * framebuffer several more times after VI init, so anything laid down
 * earlier is painted over. N64_VIPaintBorders() is called from the first
...
void N64_VIPaintBorders(void)
{
    static int done = 0;
    if (done)
        return;
    done = 1;
    paint_borders((u16 *)__fb0_start);
    paint_borders((u16 *)__fb1_start);
}
